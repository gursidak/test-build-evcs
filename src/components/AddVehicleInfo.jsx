...
class AddVehicleInfo extends Component {
  ...
  handleSubmit() {
    const info = {
      type: this.state.rcType,
      img: this.state.rcImage,
      ...
      <React.Fragment>
        <form onSubmit={this.handleSubmit}>
          <select
            ...
            className="evcsInput"
            onChange={(event) => {
              this.setState({ rcType: event.target.value });
            }}
          >
          ...
          <input
            ...
            className="evcsInput"
            onChange={(event) => {
              this.setState({ rcImage: event.target.files[0] });
            }}
          />

          <button className="btn btn-danger evcsButton " >
            SUBMIT
          </button>
        </form>
      </React.Fragment>
